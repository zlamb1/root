...
root_slab_alloc (root_slab_t *slab)
{
  root_node_t *node;
  if (slab == NULL)
    root_panic ("slab_alloc: null slab");
  if (!slab->cnt)
    return NULL;
  node = slab->freelist;
...
root_slab_free (root_slab_t *slab, void *p, int release)
{
  ...
  if (slab == NULL)
    root_panic ("slab_free: null slab");
  if (p == NULL)
    root_panic ("slab_free: null pointer");
  if (slab->cnt == slab->cap)
    root_panic ("slab_free: full slab");
  was_empty = !slab->cnt;
...
root_slab_release (root_slab_t *slab)
{
  ...
  if (slab == NULL)
    root_panic ("slab_release: null slab");
  ...
  page = root_get_page ((void *) page_address);
  if (page == NULL)
    root_panic ("slab_release: null page");
  if (!(page->flags & ROOT_PG_FLAG_SLAB))
    root_panic ("slab_release: invalid slab flag");

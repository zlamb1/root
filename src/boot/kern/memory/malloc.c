...
root_malloc (root_size_t sz)
{
  if (sz > 2048)
  ...
  else
    {
      ...
      if (index >= MAX_SLABS)
        root_panic ("malloc: invalid index");
      slab = slabs[index];
      if (slab == NULL)
        {
      ...
      if (!slab->cnt)
        root_panic ("malloc: exhausted slab");
...
root_realloc (void *p, root_size_t sz)
{
  ...
  if (page->flags & ROOT_PG_FLAG_FREE)
    root_panic ("realloc: double free detected");
  np = root_malloc (sz);
  if (np == NULL)
    return NULL;
  if (page->flags & ROOT_PG_FLAG_SLAB)
    {
      ...
      if (slab == NULL)
        root_panic ("realloc: invalid slab");
...
root_free (void *p)
{
  ...
  if (page == NULL)
    root_panic ("free: invalid page");
  if (page->flags & ROOT_PG_FLAG_FREE)
    root_panic ("free: double free detected");
  if (page->flags & ROOT_PG_FLAG_SLAB)
    {
      ...
      int found = 0;
      if (slab == NULL)
        root_panic ("free: invalid slab");
      if (!(slab->flags & ROOT_SLAB_FLAG_MALLOC))
        root_panic ("free: non-malloc slab");
      index = root_get_slab_idx (slab->sz);
      if (index >= MAX_SLABS)
        root_panic ("free: invalid index");
      iter = slabs[index];
      while (iter != NULL)
        {
          if (iter == slab)
